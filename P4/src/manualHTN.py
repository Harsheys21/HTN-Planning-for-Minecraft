...
def op_craft_plank(state, ID):		#operator to make 4 planks of wood with one peice of wood
...
def op_craft_stick(state, ID):
	if state.plank[ID] >= 2 and state.time[ID] >= 1:
		state.stick[ID] += 4
		state.plank[ID] -= 2
		state.time[ID] -= 1
		return state
...
pyhop.declare_operators (op_punch_for_wood, op_craft_plank, op_craft_bench, op_craft_stick, op_wooden_axe_for_wood, op_craft_wooden_axe_at_bench)
